# Donald B. Havery

**AI Systems Engineer | Full-Stack Developer**

I build production-grade AI systems — from real-time voice assistants with multi-agent orchestration to enterprise SaaS platforms with 6 role-based portals. 30 years and 3.5M+ miles in transportation operations built the systems thinking; 930+ commits and 170K+ lines of code turned it into software.

Currently open to opportunities in AI engineering, full-stack development, and systems architecture.

---

### What I Build

<table>
<tr>
<td width="50%">

**[Vox](https://github.com/dbhavery/vox)** — AI Voice Assistant Framework

Multi-agent orchestration with 7 specialized agents, 5-model LLM routing, real-time voice pipeline (449ms p50 latency), photorealistic avatar animation. Python, asyncio, WebSocket. **700+ tests.**

</td>
<td width="50%">

**[Cortex](https://github.com/dbhavery/cortex)** — AI Media Cataloger

Autonomous media indexing with BLIP captioning, InsightFace face clustering, Google Photos/Drive sync, semantic search. FastAPI + React. **5,700+ items processed.**

</td>
</tr>
<tr>
<td width="50%">

**[CodeRev](https://github.com/dbhavery/coderev)** — AI Code Review Bot

GitHub App that uses Claude to review PRs — finds bugs, security issues, suggests improvements. Dashboard with review metrics. FastAPI + React + Claude API.

</td>
<td width="50%">

**[Ralph for Claude Code](https://github.com/dbhavery/ralph-claude-code)** — Autonomous Dev Loop

Intelligent exit detection, circuit breaker patterns, dual-condition gates, session continuity. **566 passing tests.** Featured in [Awesome Claude Code](https://github.com/hesreallyhim/awesome-claude-code).

</td>
</tr>
<tr>
<td width="50%">

**[StaFull Platform](https://github.com/dbhavery/stafull-platform)** — Enterprise SaaS

Mobile fuel delivery platform — 6 portals, 9 user roles, real-time fleet tracking, 255-input simulation engine. React, Node.js, PostgreSQL, Socket.io. **Deployed on Railway + Vercel.**

</td>
<td width="50%">

**[Portfolio](https://github.com/dbhavery/portfolio)** — donhavery.dev

Personal site showcasing projects, architecture deep-dives, and case studies. Built with Next.js and a custom neumorphic design system.

</td>
</tr>
</table>

---

### Tech Stack

**AI/ML** &nbsp; `Claude` `Gemini` `Ollama` `PyTorch` `HuggingFace` `ChromaDB` `RAG` `LangGraph`

**Backend** &nbsp; `Python` `FastAPI` `asyncio` `Node.js` `Express` `WebSocket` `REST APIs`

**Frontend** &nbsp; `React` `TypeScript` `Next.js` `Tailwind` `PySide6`

**Data** &nbsp; `PostgreSQL` `SQLite` `ChromaDB` `Prisma ORM` `Peewee ORM`

**DevOps** &nbsp; `Docker` `GitHub Actions` `Vercel` `Railway` `CI/CD`

**Voice & Vision** &nbsp; `Whisper STT` `Chatterbox TTS` `Silero VAD` `InsightFace` `LivePortrait` `BLIP`

---

### GitHub Stats

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=dbhavery&show_icons=true&theme=dark&hide_border=true&count_private=true&bg_color=0d1117" alt="GitHub Stats" height="165" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=dbhavery&layout=compact&theme=dark&hide_border=true&bg_color=0d1117&langs_count=8" alt="Top Languages" height="165" />
</p>

<p align="center">
<img src="https://github-readme-streak-stats.herokuapp.com/?user=dbhavery&theme=dark&hide_border=true&background=0d1117" alt="GitHub Streak" />
</p>

---

### Contact

**Email:** dbhavery@gmail.com &nbsp;|&nbsp; **GitHub:** [github.com/dbhavery](https://github.com/dbhavery) &nbsp;|&nbsp; **Resume:** [Download PDF](https://github.com/dbhavery/dbhavery/raw/main/Donald_Havery_Resume.pdf)
